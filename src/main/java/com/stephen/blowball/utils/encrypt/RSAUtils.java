package com.stephen.blowball.utils.encrypt;
...
public class RSAUtils {
	...
	public static KeyPair generateKeyPair(int keySize) {
		try {
			if (!Objects.equals(keySize % 512, 0) || keySize < 512) {
				throw new BusinessException(ErrorCode.OPERATION_ERROR, "密钥长度必须是512的倍数，且不小于512位");
			}
			KeyPairGenerator keyPairGenerator = KeyPairGenerator.getInstance("RSA");
			keyPairGenerator.initialize(keySize);
			return keyPairGenerator.generateKeyPair();
		} catch (Exception e) {
			throw new BusinessException(ErrorCode.OPERATION_ERROR, "生成RSA密钥对出错");
		}
	}
	...
	public static PublicKey getPublicKeyFromString(String keyString) {
		try {
			...
			throw new BusinessException(ErrorCode.OPERATION_ERROR, "从字符串获取公钥出错" + e.getMessage());
		}
	}
	...
	public static PrivateKey getPrivateKeyFromString(String keyString) {
		try {
			...
			throw new BusinessException(ErrorCode.OPERATION_ERROR, "从字符串获取私钥出错" + e.getMessage());
		}
	}
	...
	public static String getStringFromPublicKey(PublicKey publicKey) {
		try {
			...
			throw new BusinessException(ErrorCode.OPERATION_ERROR, "将公钥转换为字符串出错" + e.getMessage());
		}
	}
	...
	public static String getStringFromPrivateKey(PrivateKey privateKey) {
		try {
			...
			throw new BusinessException(ErrorCode.OPERATION_ERROR, "将私钥转换为字符串出错" + e.getMessage());
		}
	}
	...
	public static String encrypt(String plaintext, PublicKey publicKey) {
		try {
			...
			throw new BusinessException(ErrorCode.OPERATION_ERROR, "公钥加密出错" + e.getMessage());
		}
	}
	...
	public static String decrypt(String ciphertext, PrivateKey privateKey) {
		try {
			...
			throw new BusinessException(ErrorCode.OPERATION_ERROR, "私钥解密出错" + e.getMessage());
		}
	}
