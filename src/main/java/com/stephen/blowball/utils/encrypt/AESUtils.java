...
public class AESUtils {
	...
	public static SecretKey generateKey(int keySize) {
		try {
			if (!Objects.equals(keySize * 8, 128)
				...
				throw new BusinessException(ErrorCode.OPERATION_ERROR, "无效的AES密钥长度（需要16/24/32字节）");
			}
			KeyGenerator keyGenerator = KeyGenerator.getInstance("AES");
			keyGenerator.init(keySize * 8);
			return keyGenerator.generateKey();
		} catch (NoSuchAlgorithmException e) {
			throw new BusinessException(ErrorCode.OPERATION_ERROR, "生成AES密钥出错");
		}
	}
	...
	public static SecretKey getKeyFromString(String keyString) {
		if (!Objects.equals(keyString.length(), 16)
			...
			throw new BusinessException(ErrorCode.PARAMS_ERROR, "无效的AES密钥长度（需要16/24/32字节）");
		}
	...
	public static String getStringFromKey(SecretKey secretKey) {
		if (ObjectUtils.isEmpty(secretKey)) {
			throw new BusinessException(ErrorCode.PARAMS_ERROR, "密钥不能为空");
		}
	...
	public static String encrypt(String plaintext, SecretKey secretKey) {
		try {
			...
			throw new BusinessException(ErrorCode.OPERATION_ERROR, "密钥加密出错" + e.getMessage());
		}
	}
	...
	public static String decrypt(String ciphertext, SecretKey secretKey) {
		try {
			...
			throw new BusinessException(ErrorCode.OPERATION_ERROR, "密钥解密出错" + e.getMessage());
		}
	}
